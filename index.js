 ...
 */let stringnum= "1999";
   let integernum= Number(stringnum);
 ...
 */
 ...
 function bilgisayarinSecimi() {
   ...
   const rastgeleSayi = Math.floor(Math.random() * secenekler.length);
   return secenekler[rastgeleSayi];
 }
 const oyuncuSecimi = "Makas"; // Oyuncunun seçimini burada belirleyebilirsiniz
 const bilgisayarSecimi = bilgisayarinSecimi(); // Bilgisayarın rastgele seçimi
 const sonuc = oyun(oyuncuSecimi, bilgisayarSecimi); // Sonucu hesapla
 
 console.log(`Oyuncu: ${oyuncuSecimi}, Bilgisayar: ${bilgisayarSecimi}`);
 console.log(sonuc);
 ...
 */
 ...
 function sesliHarfSayaci() {
   let ismim= ("Sejda Ozgur Ficici");
   let ismimkucuk= ismim.toLowerCase().join();
   let sesliharfler= 'aeiou';
   toplamSesli = 0;
   const sesliharf= ismimkucuk.forEach((harf) => {
     if (sesliharfler.includes(harf)) {
       toplamSesli= toplamSesli + 1;
     }
   }
   )
